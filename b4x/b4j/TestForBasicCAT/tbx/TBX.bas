...
Sub readTermsIntoList(filepath As String,sourceLang As String,targetLang As String)
	filepath=File.Combine(File.DirApp,"small.tbx")
	Dim termsMap As Map
	termsMap.Initialize
	...
	For Each termEntry As Map In termEntries
		...
		For Each langSet As Map In langSets
			...
			End If
		Next
		targetMap.Put(target,description)
